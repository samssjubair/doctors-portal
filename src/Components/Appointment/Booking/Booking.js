...
const Booking = ({date}) => {
    return (
        <div>
            <h2 className="text-primary text-center m-5">Available Appointment on {date.toDateString()}</h2>
            <div className="row">
                {
                    bookingData.map(book=> <BookingCard key={book._id} date={date} book={book}></BookingCard> )
                }
            </div>
        </div>
    );
